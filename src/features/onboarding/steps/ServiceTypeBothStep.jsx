...
const ServiceTypeBothStep = ({ onNext, onBack, onStepChange }) => {
  const { kycOptions, setKycOptions, kybOptions, setKybOptions, setSelectedServices } = useOnboarding();
  ...
  const [selectedKycIds, setSelectedKycIds] = useState(Array.isArray(kycOptions) ? kycOptions : []);
  const [selectedKybIds, setSelectedKybIds] = useState(Array.isArray(kybOptions) ? kybOptions : []);
  ...
  const currentSelectedIds = activeTab === 'kyc' ? selectedKycIds : selectedKybIds;
  ...
  return (
    <div className="plan_type">
      <div className="steps_container">
        {ACCOUNT_TYPE_STEPS.map((step, idx) => (
          <div key={idx} className={`step_item ${idx <= currentSubStep ? 'active' : ''}`}>
             <div className="dot">
                <img src={selectBoxFill} alt="select-box" className="active_dot" />
                <img src={selectBoxInactive} alt="select-box" className="inactive_dot" />
             </div>
      ...
      <div className="step_title">
        <p className="step_title_text_bg">Select Service Type - Both KYC & KYB</p>
        <p className="step_title_text_sm">Choose the type of verification services you need for your business.</p>
      </div>
      ...
      <div className="service_step_body_kyc">
        <div className="service_selector">
            <div className="service_selector_dropdown">
                ...
                <div style={{ position: 'relative' }}>
                    <select id="service_selector" defaultValue="both" onChange={handleSwitchService}>
                        <option value="" disabled>--- Switch Option ---</option>
                        <option value="kyc">KYC Only</option>
                        <option value="kyb">KYB Only</option>
                        <option value="both">Both KYC & KYB</option>
                    </select>
                    <span className="material-symbols-outlined select_chevron">expand_more</span>
                </div>
            </div>

            <div className="select_all" onClick={handleSelectAll} style={{ cursor: 'pointer' }}>
                 {currentSelectedIds.length === currentServices.length && currentServices.length > 0 && (
                    <img 
                      src={checkboxGreen} 
                      alt="Checkbox" 
          ...
          {currentServices.map((service) => {
            ...
            return (
              <div 
                ...
                onClick={() => (activeTab === 'kyc' ? toggleKycService(service.id) : toggleKybService(service.id))}
              >
                <p className="service_selector_list_item_content_title">{service.title}</p>
                ...
                <div className="service_selector_list_item_icon">
                    {!isSelected && <span className="material-symbols-outlined selection-arrow">south_east</span>}
                    {isSelected && <img src={checkboxGreen} alt="Checkbox Active" className="checkbox-active" style={{ display: 'block' }} />}
                </div>
              </div>
            );
          })}
        ...
        <div className="selector_counter">
            <p className="selector_counter_text">{currentSelectedIds.length} of {currentServices.length} selected</p>
        </div>
      </div>
